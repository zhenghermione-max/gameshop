const STORAGE_KEY = "wooshpay-gameshop-lite-v2";
...
function renderShopProducts() {
  const products = activeProducts();
  elements.shopProducts.innerHTML = products.map((product) => {
    const oldPrice = product.oldPriceUsd ? `<small>${formatMarket(product.oldPriceUsd)}</small>` : "";
    return `
      <button class="catalog-product ${product.id === state.selectedProductId ? "is-active" : ""}" type="button" data-shop-product="${product.id}">
        <span class="product-badge">${product.tag}</span>
        <span class="product-token">${product.token}</span>
        <span class="product-copy">
          <strong>${product.name}</strong>
          <small>${product.type} · ${product.bonus}</small>
          <em>${product.benefit}</em>
        </span>
...
function renderProductAdmin() {
  elements.productRows.innerHTML = state.products.map((product) => `
    <div class="table-row product-record ${product.id === state.selectedAdminProductId ? "is-selected" : ""}" data-product-id="${product.id}" role="row">
      ...
      <span>${product.status === "active" ? "已上架" : "草稿"}</span>
...
function checkout() {
  ...
  const order = {
    ...
    market: state.market,
    totalUsd: Number(cart.totalUsd.toFixed(2)),
...
function bindStaticCardSelection() {
  document.querySelectorAll(".insight-grid article, .capability-grid article, .loop div, .kpi-grid article, .chart-card, .banner-queue div").forEach((card) => {
